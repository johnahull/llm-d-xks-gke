...
kubectl apply -f huggingface-token-secret.yaml -n ${NAMESPACE}
...
```bash
# Edit the YAML file
nano huggingface-token-secret.yaml
...
Location: `llm-d-pattern1-values.yaml`
...
```bash
cat llm-d-pattern1-values.yaml
```
...
kubectl apply -f huggingface-token-secret.yaml -n llm-d
...
helm upgrade --install gaie-pattern1 \
  ...
  --version v1.2.0 \
  --namespace llm-d \
  --values ../llm-d/guides/inference-scheduling/gaie-inference-scheduling/values.yaml \
...
- Review existing RHAIIS setup: `cluster-setup.md`
- Check benchmarking guide: `benchmarks.md`
